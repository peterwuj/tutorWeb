// ninja-avatar.js – Lottie-powered ninja avatar for Ninja Learn
(function () {
  ...
  const TELEPORT_SPOTS = [
    { top: '35px',    left: '20px'  },                                   // top-left
    { top: '35px',    right: '20px' },                                   // top-right
    { bottom: '16px', left: '20px'  },                                   // bottom-left
    { bottom: '16px', right: '20px' },                                   // bottom-right
    { top: '35px',    left: '50%',  transform: 'translateX(-50%)' },    // top-center
...
`;
  ...
  // Load the Lottie library from CDN. If it fails (offline, blocked CDN,
  // local file preview) or takes too long, fall back so the avatar is
  // still visible as a static emoji ninja.
  function loadLottie(cb, onFail) {
    ...
    const timer = setTimeout(() => done(!!window.lottie), 6000);
  ...
  class NinjaAvatar {
    constructor(container, { size = 120, pose = 'idle', spotIndex = 3 } = {}) {
      ...
      // Click-to-teleport works with or without Lottie
      wrap.addEventListener('click', () => this._fogTeleport());

      // Start in the requested pose right away — the pose animations are
      // pure CSS, so the avatar bobs/waves even before (or without) Lottie.
      ...
      loadLottie(() => {
        ...
        // Fallback: static emoji ninja so the avatar is never invisible
        lottiDiv.textContent      = '🥷';
        lottiDiv.style.fontSize   = Math.round(size * 0.74) + 'px';
        lottiDiv.style.lineHeight = size + 'px';
        lottiDiv.style.textAlign  = 'center';
      });
    }
